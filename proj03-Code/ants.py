...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive:
            if self.min_range <= trans_cnt and trans_cnt <= self.max_range and cur_place.bees:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # do NOT forget to use the `list()` to copy the list being iterated
        for bee in list(self.place.bees):
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        # insect.add_to(self)           # Wrong!! this is repteating code from elsewhere
        super().add_insect(insect)      # Correct!! we should inherit instead!!
        ...
        while cur_place != None:
            ...
            if cur_place.ant:
                ...
                if cur_place.ant.is_container and cur_place.ant.ant_contained:
                    cur_place.ant.ant_contained.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        #     if self.slow_time > 0 and (gamestate.time & 1):
        #         pass
        #     else:
        #         if
        ...
        if self.blocked():
            ...
            if self.slow_time > 0 and (gamestate.time & 1):
                ...
                if self.scare_round > 0:
                    self.scare_round -= 1 # succeed to try, not meaning exactly take the move
        ...
        self.slow_time = max(0, self.slow_time - 1)
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not self.ever_scared:
            self.scare_round = length
            self.ever_scared = True
        # END Problem EC
    ...
    def insects_in_front(self):
        ...
        while cur_place and not cur_place.is_hive:
            ...
            if cur_place.ant and cur_place.ant is not self:
                res[cur_place.ant] = dist
                if cur_place.ant.is_container and cur_place.ant.ant_contained:
                    res[cur_place.ant.ant_contained] = dist
