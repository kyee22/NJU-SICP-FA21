...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    print('DEBUG:','scheme_eval called!',expr.__repr__())
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        return scheme_apply(
            ...
            rest.map(lambda x,env=env: scheme_eval(x, env)),
            env
        )
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    def pairs2list(p):
        return [] if p is nil else [p.first] + pairs2list(p.rest)

    validate_procedure(procedure)
    if isinstance(procedure, BuiltinProcedure):
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        args_lst = pairs2list(args)
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions is not nil:
            print('DEBUG:','first:',expressions.first, expressions.first.__repr__())
