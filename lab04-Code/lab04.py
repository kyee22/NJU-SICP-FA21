...
def totals_tree(m):
    ...
    # attention! here we should call `totals_tree()` instead of `tree()`
...
def has_path(t, word):
    ...
    # write a hepler function to recursive search until leaf
    def search(sub_tree, remain_str):
        if label(sub_tree) != remain_str[0]:
            return False

        # if we can decide in this recursion layer, then do it!
        # i.e., `len == 1` is the base case.
        if len(remain_str) == 1:
            return True
        # else continue recursive search...
        else:
            for b in branches(sub_tree):
                if search(b, remain_str[1:]):
                    return True
            return False

    return search(t, word)
