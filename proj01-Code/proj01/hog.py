...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if outcome == 1:
            pig_flag = True
        tot += outcome
        if (outcome & 1) == cur_parity:
            bonus += 1

    return (1 if pig_flag else tot) + bonus
...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        idx = score % 6
        idx = 6 if idx == 0 else idx
        return int(1/21 * pow(10, idx)) % 10
    # END PROBLEM 2
...
def swine_swap(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    pow3_of_sum = pow(3, player_score + opponent_score)
    s = str(pow3_of_sum)
    return s[0] == s[-1]
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if delta > running_high:
            print( f'{delta} point(s)! That\'s a record gain for Player {who}!')

        return announce_highest(who, score, max(running_high, delta))
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    max_score_nroll = 0
    for i in range(1, 11):
        tmp_score = averaged_dice(current_score, i, dice)
        if tmp_score > max_score:
            max_score = tmp_score
            max_score_nroll = i

    return max_score_nroll
    # END PROBLEM 9
...
def swine_swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # return 6  # Remove this line once implemented.
    cur_score, opp_score = score + picky_piggy(opponent_score), opponent_score
    if swine_swap(cur_score, opp_score):
        swapped_cur_score, swapped_opp_score = opp_score, cur_score
        if swapped_cur_score - cur_score >= cutoff:
            return 0
        else:
            return num_rolls
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    # return 6  # Remove this line once implemented.
    return picky_piggy_strategy(score, opponent_score, 5, 3)
    # END PROBLEM 12
